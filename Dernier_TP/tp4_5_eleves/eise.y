...
%token T_SKIP T_THEN T_VAR T_VARIABLE T_WHILE T_WRITE
...
program_eise: T_PROGRAM T_VARIABLE ';' bloc_principal '.' { programRoot = $4; }
	; 
...
appel_fonction : T_VARIABLE '(' liste_parametres_effectifs ')' 
		{ 
			...
			if (varglob!=-1)
				$$ = createOperatorNode(OPER_APPEL_FONCTION,2,createIdentifierNode($1,table_ident_fonctions[0][varglob].funcNum,varglob),$3) ; 
			else
			{
...
liste_parametres_effectifs : liste_parametres_effectifs ',' expr { $$ = createOperatorNode(OPER_SEQUENCE,2,$1,$3) ; }
        | expr 
		  |	{ $$ = createOperatorNode(OPER_SKIP,0); }
        ;
